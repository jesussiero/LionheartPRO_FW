...
extern bool flag_level_bed_process; //SAG20160128
extern bool flag_change_filament;  //SAG20160201
...
extern void lcd_led_on();
extern void lcd_led_off();

#if HAS_KILL || HAS_HOME
//SAG20150811
#define _MAX_CNT_DOWN 12000 // Press counter
#define _MAX_CNT_UP 750     // Release counter
#endif
...
bool flag_print_finish = false; //SAG20150819
bool flag_print_start = false; //SAG20150819
...
//SAG20160719
  ...
  //SAG20150818
  enqueuecommands_P(PSTR("M117 " CUSTOM_MACHINE_NAME " FW " FIRMWARE_VERSION "\n")); // Show FW version message (Message at boot)
...
void get_command() {
  ...
  while (commands_in_queue < BUFSIZE && MYSERIAL.available() > 0) {
    ...
    while (!card.eof() && commands_in_queue < BUFSIZE && !stop_buffering) {
	  ...
	  if (!card.cardOK) return; // SAG20150819 (For safety reasons / inactivity timer can work if SD removal)
...
inline void gcode_G92() {
  ...
  inline void gcode_M0_M1() {
    ...
    if (!hasP && !hasS && *args != '\0')
      ...
      if(!flag_level_bed_process) LCD_MESSAGEPGM(MSG_USERWAIT); //SAG20160128
...
inline void gcode_M42() {
  if (code_seen('S')) {
    ...
    if (pin_number > -1) {
      ...
      //analogWrite(pin_number, pin_status); //SAG20150812 (issue with Teensyduino and Arduino for PWM ports)
	  	  
	  if(pin_number == LED_PIN) led_status = pin_status; //SAG20150812
	  else if(pin_number != FAN2_PIN) analogWrite(pin_number, pin_status); //SAG20150813
...
inline void gcode_M104() {
  ...
  if (code_seen('S')) {
	...
	//SAG20150819
	if(flag_print_start) {
		if(temp == 0) { //Printing finishes
			flag_print_finish = true;
			flag_print_start = false;			
		}
	}
  }
}
...
inline void gcode_M109() {
  ...
  //SAG20150819
  flag_print_start = true;
  enqueuecommands_P(PSTR("M85 S600")); //Set inactivity shutdown timer (10 min)
}
      ...
      switch (material) {
        case 0:
          if (code_seen('H')) {
			...
			plaPreheatHotendTemp = constrain(v, EXTRUDE_MINTEMP, HEATER_0_MAXTEMP - 5); //SAG20160122
          }
          if (code_seen('F')) {
            v = code_value_short();
            ...
            if (code_seen('B')) {
			  ...
			  plaPreheatHPBTemp = constrain(v, BED_MINTEMP, BED_MAXTEMP - 5); //SAG20160122
            }
          #endif
          break;
        case 1:
          if (code_seen('H')) {
			...
			absPreheatHotendTemp = constrain(v, EXTRUDE_MINTEMP, HEATER_0_MAXTEMP - 5); //SAG20160122
          }
          if (code_seen('F')) {
            v = code_value_short();
            ...
            if (code_seen('B')) {
			  ...
			  absPreheatHPBTemp = constrain(v, BED_MINTEMP, BED_MAXTEMP - 5); //SAG20160122
            }
          #endif
          break;
      }
...
#ifdef LED_PIN
 /**
   * M599: Blink LED Px Nx (Custom León 3D)
   *
   *  P[ms] - Blinking half-period
   *  N[times] - Number of blinks
   *  Default values are used for omitted arguments.
   *
   */
inline void gcode_M599() {

  millis_t t_period = 500;
  millis_t t_period_tmp = 0;
  char num_blinks = 5;  
  ...
  for (char i = 0; i < num_blinks; i++) {
    t_period_tmp = millis() + t_period;
    digitalWrite(LED_PIN,1);
    while (millis() < t_period_tmp) idle();
    t_period_tmp = millis() + t_period; 
    digitalWrite(LED_PIN,0);
    while (millis() < t_period_tmp) idle();
  }

  //Restore led status as per user configuration
  if (led_status == 0) lcd_led_off();
  else lcd_led_on();
...
void set_M109(float temp) {
  ...
  inline void gcode_M600() {
    //SAG20160203
    // Routine completely rewritten
    float filament_final_retract = 0;
    flag_change_filament = true;
    
    float t_hotend_backup = 0;
    float t_bed_backup = 0;
    char t_fan = 0;
    
    //Info for user
     #ifndef LANGUAGE_ES_H
      LCD_MESSAGEPGM("Filament change");
    #else
      LCD_MESSAGEPGM("Cambio de filamento");	
    #endif
    lcd_update();
     ...
     if (code_seen('E')) {
      ...
      if (code_seen('Z')) {
        target[Z_AXIS] += code_value();
          ...
          if(current_position[Z_AXIS] > 8.0 ) target[Z_AXIS] += FILAMENTCHANGE_ZADD; //SAG20160718
          else target[Z_AXIS] += FILAMENTCHANGE_ZADD2;                               //SAG20160718
      ...
      plan_buffer_line(target[X_AXIS], target[Y_AXIS], target[Z_AXIS], target[E_AXIS], 4800/60, active_extruder);
      st_synchronize();
      enable_x();
      enable_y();
    ...
    //Press to start
    #ifndef LANGUAGE_ES_H
      LCD_MESSAGEPGM("Press to start");
    #else
      LCD_MESSAGEPGM("Pulsa para empezar");	
    #endif
    
    millis_t t_next_event = 0;
    millis_t t_watch_dog = 0; //SAG20160727
    millis_t t_watch = millis() + 600000; //10 minutes
    char flag_watch_dog = 0;
    char _status = 0;
    lcd_ignore_click();
    while (!lcd_clicked()) {
        manage_heater();
        //manage_inactivity(); //SAG20160727
        lcd_update();
        ...
        #if LED_PIN > -1
          pinMode(LED_PIN, OUTPUT);
          digitalWrite(LED_PIN, (_status) ? 1 : 0);
          _status = !_status; 
        #endif   
        if(flag_watch_dog == 0) t_next_event = ms + 200; // toggle LED every 200 ms
        else t_next_event = ms + 1000; // toggle LED every 1000 ms
       }
       
       if(flag_watch_dog == 0)
         {
           millis_t t_watch_dog = millis();
           if (t_watch_dog >= t_watch) //SAG20160727
             {
                t_hotend_backup = degTargetHotend(active_extruder);
                t_bed_backup = degTargetBed();
                t_fan = fanSpeed;
                
                disable_all_heaters();
                
                fanSpeed = 0; //Disable fan                
                check_axes_activity(); //SAG20160805
    ...
    if(flag_watch_dog)
      {
        ...
        refresh_cmd_timeout(); //SAG20160728
        
        fanSpeed = t_fan; // Set fan
        check_axes_activity(); //SAG20160805
        
        set_M190(t_bed_backup); // Set bed temp and wait
        set_M109(t_hotend_backup); //Set hotend temp and wait
      }
    
    //Unload filament
    #ifndef LANGUAGE_ES_H
      LCD_MESSAGEPGM("Press after unload");
    #else
      LCD_MESSAGEPGM("Pulsa tras extraer");
    #endif
    lcd_update();
    
    if (code_seen('L')) {
        filament_final_retract = code_value();
      } else {
        #ifdef FILAMENTCHANGE_FINALRETRACT
          filament_final_retract = FILAMENTCHANGE_FINALRETRACT;
        #endif // FILAMENTCHANGE_FINALRETRACT
      }
    
    plan_buffer_line(target[X_AXIS], target[Y_AXIS], target[Z_AXIS], target[E_AXIS]+15, 120/60, active_extruder);                      // Extrude 15 mm of material
    plan_buffer_line(target[X_AXIS], target[Y_AXIS], target[Z_AXIS], target[E_AXIS]+filament_final_retract, 3000/60, active_extruder); // Retract 220 mm of material (fast)
    st_synchronize();
    ...
    //Load new filament
    #ifndef LANGUAGE_ES_H
      LCD_MESSAGEPGM("Press after load");
    #else
      LCD_MESSAGEPGM("Pulsa tras la carga");	
    #endif
    lcd_update();
    ...
    while(!lcd_clicked()){
      ...
      if(!blocks_queued()) {
       current_position[E_AXIS]+=0.01;
       plan_buffer_line(target[X_AXIS], target[Y_AXIS], target[Z_AXIS],current_position[E_AXIS], 180/60, active_extruder); //SAG20160307
      }
    ...
    plan_buffer_line(lastpos[X_AXIS], lastpos[Y_AXIS], target[Z_AXIS], lastpos[E_AXIS], 4800/60, active_extruder); //move xy back
    plan_buffer_line(lastpos[X_AXIS], lastpos[Y_AXIS], target[Z_AXIS], lastpos[E_AXIS] - 0.1, 1200/60, active_extruder); //Compensate E axis (experimental //SAG20160309
    ...
    enqueuecommands_P(PSTR("M601")); // End of change filament process (clear flag_change_filament)
    st_synchronize();
  }
...
void process_next_command() {
  ...
  switch(command_code) {
    ...
    case 'M': switch (codenum) {
      ...
      //SAG20160128
      case 700: 
        gcode_M700(); //M700 Custom G-CODE: notify end of leveling base process
        break;
...
#if HAS_KILL
//SAG20150811
char checkKillButton(void)
{
  /* Kill button */
  static char flag_kill;
  static char kill_disable = 0;
  static int cnt_kill_down = _MAX_CNT_DOWN;
  static int cnt_kill_up = _MAX_CNT_UP;
  ...
  if(!READ(KILL_PIN))
	...
	if(!kill_disable)
	  {
	    if(--cnt_kill_down==0)	/* Debounce */
		{					
		  cnt_kill_up=_MAX_CNT_UP;
		  kill_disable=1; /* Inhibit button sense until release */
		  flag_kill=1; /* Notify Event */
	    }
	  }
	else cnt_kill_up=_MAX_CNT_UP; /* Waiting for release */
      }			
    else
      {
        cnt_kill_down=_MAX_CNT_DOWN;	/* Glitch */
	if(cnt_kill_up!=0)
	  {
	    if(--cnt_kill_up==0) kill_disable=0; /* Released */
	  }		
      }
...
#if HAS_HOME
//SAG20150811
char checkHomeButton(void)
{
  /* Home button */
  static char flag_home;
  static char home_disable = 0;
  static int cnt_home_down = _MAX_CNT_DOWN;
  static int cnt_home_up = _MAX_CNT_UP;
  ...
  if (!READ(HOME_PIN))
	...
	if(!home_disable)
	  {
	    if(--cnt_home_down==0)	/* Debounce */
		{					
		  cnt_home_up=_MAX_CNT_UP;
		  home_disable=1; /* Inhibit button sense until release */
		  flag_home=1; /* Notify Event */
	    }
	  }
	else cnt_home_up=_MAX_CNT_UP; /* Waiting for release */
      }			
    else
      {
        cnt_home_down=_MAX_CNT_DOWN;	/* Glitch */
	if(cnt_home_up!=0)
	  {
	    if(--cnt_home_up==0) home_disable=0; /* Released */
	  }		
      }
...
void manage_inactivity(bool ignore_stepper_queue/*=false*/) {
  ...
  #if HAS_FILRUNOUT
    if ((movesplanned() || IS_SD_PRINTING) && !(READ(FILRUNOUT_PIN) ^ FIL_RUNOUT_INVERTING)) //SAG20160718
      filrunout();
  #endif
  ...
  millis_t ms = millis();
   ...
   if (max_inactive_time && ms > previous_cmd_ms + max_inactive_time) {
	//SAG20150819
	max_inactive_time = 0; //reset inactivity timer	
	if(flag_print_finish) {
		#ifdef LED_OFF_WHEN_PRINTING_FINISHES
		//Switch off printer LED			
			#if LED_PIN > -1
				pinMode(LED_PIN, OUTPUT);
				digitalWrite(LED_PIN,0);
				led_status = 0;	
			#endif
		#endif
		flag_print_finish = false;
		}
	else { //Inactivity timer expired, but it was not after a successful printing		
			//Deactivate FAN1, FAN2 & LED
			#if FAN_PIN > -1
			pinMode(FAN_PIN, OUTPUT); //FAN1 OFF
			digitalWrite(FAN_PIN,0);
			#endif
			#if FAN2_PIN > -1
			pinMode(FAN2_PIN, OUTPUT); //FAN2 OFF
			digitalWrite(FAN2_PIN,0);
			#endif
			#if LED_PIN > -1
			pinMode(LED_PIN, OUTPUT); //LED OFF
			digitalWrite(LED_PIN,0);
			#endif
...
//SAG20150811
	if (checkKillButton()) {
		kill(PSTR(MSG_KILLED)); // Kill button pressed
	}
...
//SAG20150811
	if (checkHomeButton()) {	
		enqueuecommands_P(PSTR("G28")); // Home button pressed
